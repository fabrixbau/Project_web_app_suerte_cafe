...
def customization_group_library(exclude_product=None):
    # Varios productos pueden tener un grupo con el mismo nombre (por ejemplo "Leche") porque
    # se copiaron unos de otros en su momento. La lista para "Asociar grupo" muestra cada
    # nombre una sola vez, no una fila repetida por cada producto que lo tiene.
    groups = ProductOptionGroup.objects.select_related("product").prefetch_related(
        "options",
    ).order_by("name", "product__name", "sort_order")
    if exclude_product:
        groups = groups.exclude(product=exclude_product)
    unique_groups = {}
    for group in groups:
...
def sync_product_customization(product, clean_groups):
    # Reserva nombres internos temporales para evitar choques de unicidad cuando un grupo
    # pegado toma el nombre que otro grupo del mismo producto está dejando de usar.
    existing_groups = {
    ...
    for group_data in clean_groups:
        ...
        for option_data in group_data["options"]:
            ...
            if option_id:
                ...
                for field_name in ("name", "price_adjustment", "is_default", "is_available", "replacement_pair", "sort_order"):
                    setattr(option, field_name, option_data[field_name])
